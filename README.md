...
문서를 내 PC에서 읽어 MP3로 만드는 웹 앱입니다. **배포된 사이트에 접속해 파일을 선택하면, 문서와 생성 음성을 외부 서버에 업로드하지 않고 내 브라우저에서 처리합니다.** 기본 웹 모드에는 Python이나 별도 실행 프로그램이 필요하지 않습니다.

## 웹에서 사용하기

1. 배포된 사이트의 **모델 보관함**에서 Supertonic 3을 한 번 다운로드합니다. 약 401 MB가 내 PC의 브라우저 전용 저장소에 저장됩니다.
2. **새 음성 만들기**에서 PDF·DOCX·DOC·UTF-8 TXT를 선택하거나 글을 입력합니다. 파일 선택은 내 PC의 파일을 브라우저가 읽도록 허용하는 동작입니다.
3. 목소리를 고르고 **샘플 듣기**로 비교한 뒤 변환을 시작합니다. 샘플은 모델 다운로드 전에도 들을 수 있습니다.
4. **작업 기록**에서 진행률을 확인하고, 완성된 MP3를 내 PC에 저장합니다.

![문서 선택, 목소리 샘플, 변환 설정을 보여 주는 실제 웹 화면](docs/assets/screenshots/web/new-conversion.png)

## 내 문서는 어디서 처리되나요?

![웹 화면과 모델만 내려받고, 문서 읽기부터 MP3 저장까지 내 PC에서 처리하는 구조](docs/assets/diagrams/browser-processing.svg)

| 순서 | 내부 동작 | 실제 저장·실행 위치 |
| --- | --- | --- |
| 1. 사이트 접속 | 화면과 문서·음성 처리 프로그램을 다운로드 | 내 PC의 브라우저 캐시 |
| 2. 모델 다운로드 | 고정된 버전의 Supertonic 3 파일을 내려받아 검증 | 내 PC의 브라우저 전용 저장소인 IndexedDB |
| 3. 문서 선택 후 변환 시작 | 작업을 등록한 뒤 글자를 추출·분할, 필요하면 OCR | 등록한 원문·설정을 IndexedDB에 보관하고 내 브라우저에서 처리 |
| 4. 음성 생성 | 모델을 메모리에 불러와 문장별 음성을 생성 | 내 PC의 RAM·CPU, 브라우저 실행 엔진(WASM) |
| 5. 진행 저장 | 완료된 음성 구간과 작업 위치를 저장 | IndexedDB. 외부 계정이나 서버에 동기화하지 않음 |
| 6. MP3 저장 | 음성을 합쳐 MP3를 만들고 파일로 저장 | 먼저 IndexedDB, 저장 버튼을 누르면 다운로드 폴더 또는 브라우저에서 선택한 위치 |

**원문과 설정은 ‘음성 변환 시작’을 눌러 작업 등록이 완료된 뒤 저장됩니다.** 등록 전 선택한 파일이나 입력 중인 글은 창을 닫으면 사라집니다.

**창을 닫으면 계산은 멈춥니다.** 같은 사이트를 같은 브라우저 프로필에서 다시 열면 저장된 구간부터 이어갑니다. 직접 일시정지하거나 실패한 작업은 기록에서 이어하기·재시도를 선택하세요. 화면의 오프라인 준비와 모델 다운로드를 마치면 인터넷 없이도 변환할 수 있습니다.

**브라우저 저장소도 내 PC 디스크입니다.** 현재 웹 모드에는 모델을 원하는 일반 폴더에 저장하는 기능이 없습니다. 사이트 데이터를 삭제하면 모델·문서·작업 기록이 함께 지워질 수 있으므로 완성한 MP3는 파일로 따로 보관하세요. 저장 방식과 폴더 지정 가능 여부는 [모델 저장 위치 안내](docs/model-storage.md)에서 설명합니다.

## 화면에서 도움말 보기

사이트의 **도움말** 메뉴에서 처리 과정, 모델 저장 위치, 오프라인 사용과 이어하기를 확인할 수 있습니다. 문서로 읽으려면 [웹 사용 안내](docs/web-usage.md)를 참고하세요.

![내 PC에서 처리되는 과정과 저장 방식을 설명하는 실제 도움말 화면](docs/assets/screenshots/web/help.png)

## 웹으로 변환하기 어렵다면

**기존 Qwen3 로컬 실행 방식도 그대로 사용할 수 있습니다.** Apple Silicon Mac에서 처음 설치를 마쳤다면 프로젝트 폴더의 터미널에서 다음 명령으로 변환합니다. 기본 모델은 Qwen3-TTS 1.7B, 한국어 목소리는 Sohee입니다.

```bash
uv run doc2audio "문서.pdf" -o "결과.mp3"
```

웹 화면으로 Qwen3를 쓰려면 웹 빌드까지 준비한 뒤 `uv run doc2audio-server`를 실행하고 **http://127.0.0.1:8010/?runtime=local#new**를 엽니다. 기본 웹 화면의 **Qwen3 사용 방법 → Qwen3 로컬 화면 열기**에서도 연결할 수 있습니다. 이때의 서버는 내 Mac에서 켜 둔 실행 프로그램입니다.

| 실행 방식 | 지원 모델 | 실행·보관 위치 |
| --- | --- | --- |
| 기본 브라우저 웹 | Supertonic 3 · 10개 목소리 · 한국어 포함 31개 언어 | 내 브라우저의 CPU 실행·IndexedDB 보관 |
| 로컬 서버·CLI | Qwen3-TTS 1.7B·0.6B, Supertonic 3 | 내 Mac의 실행 프로그램·일반 폴더 보관 |

Supertonic 모델은 사용 조건이 포함된 OpenRAIL-M, Qwen 모델은 Apache-2.0입니다. 목록은 검토한 고정 카탈로그이며 새 모델을 자동 수집하지 않습니다. 로컬 모드에서 설치한 모델과 브라우저 모델 보관함은 별개입니다.

[기존 로컬 방식으로 사용하기](docs/local-usage.md) · [모델 저장 위치 안내](docs/model-storage.md)

## 웹 개발·정적 배포

아래 명령은 사이트 이용자가 아닌 **개발·운영자용**입니다. Node.js 22.20 이상(22.x), 24.12 이상(24.x), 또는 26 이상에서 프로젝트 루트 기준으로 실행합니다.
...
로컬 빌드 확인 주소는 **http://127.0.0.1:4173**입니다. 공개 서비스에는 **`apps/web/dist/` 전체**를 HTTPS 정적 호스팅에 올립니다. 웹서버는 화면과 실행 파일을 제공하고, 모델 다운로드와 문서 변환은 이용자의 브라우저가 담당합니다.

[브라우저 실행·배포 안내](docs/browser-runtime.md) · [모노레포 설계](docs/monorepo.md) · [문서 이미지 관리](docs/assets/README.md)

## Mac용 CLI 안내

아래 준비 사항과 터미널 명령은 **Python·MLX 기반 Mac용 CLI**에 해당합니다. 웹사이트 이용자는 이 설치 과정을 진행하지 않아도 됩니다.
...
**로컬 모델도 AI입니다.** 다만 ChatGPT 같은 외부 서비스에 문서를 보내 읽게 하는 방식이 아니라, 내려받은 음성 모델을 내 Mac에서 실행합니다. 일반적인 사용에는 Codex나 Claude Code도 필요하지 않습니다. 이 README의 명령을 터미널에서 실행하면 됩니다.

- 처음 사용한다면: [준비 사항](#준비-사항) → [처음 한 번 설치하기](#처음-한-번-설치하기) → [샘플 듣기](#샘플-듣기)
- 설치를 마쳤다면: [내 문서 변환하기](#내-문서-변환하기)
- 문제가 생겼다면: [문제 해결](#문제-해결)
- 모델 설치·목소리·말투 설정·Python 사용: [모델 사용 가이드](docs/model-usage.md)
- 어떤 모델을 어떻게 불러 MP3를 만드는지: [내부 동작 기술 문서](docs/architecture.md)
...
`README.md`, `pyproject.toml`, `samples`, `apps`, `packages` 등이 보이면 맞는 위치입니다. **이후의 `uv` 명령은 모두 이 프로젝트 폴더에서 실행합니다.** 터미널 창을 새로 열면 다시 이 폴더로 이동해야 합니다.
...
### input에는 어떤 파일이 필요한가요?

**명령에 지정한 파일 하나만 읽습니다.** `input/` 안의 모든 파일을 자동으로 읽거나, 같은 이름의 PDF·TXT·MD를 함께 참고하지 않습니다. `input/`은 편의를 위한 폴더 이름이며 다른 위치의 파일도 지정할 수 있습니다.

| 사용 방법 | 보관할 파일 | 실제 음성 입력 |
| --- | --- | --- |
| 원문을 바로 변환 | 원본 PDF·DOCX·DOC·TXT 중 하나 | 명령에 지정한 원문 파일 |
| PDF 본문을 정리한 뒤 변환 | 원문 확인용 PDF + 최종 낭독용 TXT | 명령에 지정한 낭독용 TXT |
| 발음 사전을 사용 | 위 파일 + 내 발음 사전 JSON | 본문과 `--pronunciations`로 지정한 사전 |

예를 들어 다음처럼 보관하면 충분합니다. 원문을 별도 폴더에 보관한다면 변환 자체에는 최종 TXT 하나만 있어도 됩니다.

```text
input/
├── 읽을 문서.pdf          내용 확인용 원문
└── 읽을 문서-낭독용.txt   실제로 읽을 글을 정리한 파일
```

```bash
uv run doc2audio "input/읽을 문서-낭독용.txt" -o "output/읽을 문서.mp3"
```

이 명령은 **TXT만 읽습니다.** PDF는 내용을 확인할 때 쓰는 원문입니다. 사용하지 않은 MD 사본, 초안 TXT, 일부 페이지만 담은 임시 TXT는 최종 본문에 필요한 내용이 있는지 확인한 후 정리해도 됩니다. 완성 MP3 재생에는 입력 문서나 모델이 필요하지 않습니다. 재변환하려면 실제 입력 파일을, 중단 후 이어하려면 중간 작업 폴더도 보관하세요.

**Markdown(`.md`)은 직접 지원하지 않습니다.** PDF 버전이 있으면 PDF를 쓰세요. MD만 있다면 읽을 본문을 복사해 일반 텍스트(UTF-8) `.txt`로 저장하고 제목 기호·링크·이미지 데이터 등을 검토하세요. 확장자만 바꾸면 Markdown 문법이나 이미지 데이터가 본문에 남을 수 있습니다.
...
저장할 때는 일반 텍스트(UTF-8)를 유지하세요. 텍스트 편집기에서 서식 있는 문서로 바뀌었다면 **포맷 → 일반 텍스트 만들기**를 선택하고 `.txt`로 저장합니다. 표·수식·날짜·금액이 중요한 문서는 본문과 미리듣기를 함께 확인하는 것이 좋습니다.

PDF의 쪽번호·반복 머리말·그림 설명도 추출된 글에 포함될 수 있습니다. 프로그램이 이런 내용을 일괄 삭제해 주지는 않습니다. 추출 결과에서 읽지 않을 부분만 직접 정리하고, 원문 PDF와 대조하세요. 최종 정리본은 `input/읽을 문서-낭독용.txt`처럼 알아보기 쉬운 이름으로 보관하고 **그 TXT 경로로 변환**하면 됩니다.
...
화자 선택, 설정별 적용 단계, 같은 본문으로 비교하는 방법은 [모델 사용 가이드](docs/model-usage.md)에 설명했습니다.
...
진행 메시지의 `[10/100]`은 **총 100개 음성 구간 중 10번째**라는 뜻이며, PDF 페이지 번호가 아닙니다. 마지막 구간 생성 뒤에도 음량 조정과 파일 결합에 시간이 필요합니다. `output`, `seconds`, `chunks` 등이 담긴 최종 결과가 나오고 터미널 입력 줄이 돌아오면 완료입니다. `seconds`는 생성 음성 길이, `elapsed_seconds`는 이번 실행에 걸린 시간입니다.
...
`input/`의 낭독용 TXT는 **읽을 내용**, `.models/`는 **목소리를 만드는 모델**, `.venv/`는 **프로그램 실행 환경**입니다. 서로 역할이 다릅니다. 설치가 끝난 뒤에는 `uv sync`나 `download`를 매번 실행하지 않고 변환 명령만 사용하면 됩니다.
...
일반 사용자는 이 절의 추가 검증 모델을 설치할 필요가 없습니다.

| 문서 | 설명 |
| --- | --- |
| [모델 사용 가이드](docs/model-usage.md) | 설치 확인, 옵션별 사용법, Python에서 문서 변환하기 |
| [내부 동작 기술 문서](docs/architecture.md) | 고정 모델·리비전, 모델 로딩과 음성 생성 호출, 본문 처리·이어하기·MP3 저장 |
| [모델 조사](docs/model-research.md) | 모델 선택 근거 |
| [실행 검증](docs/validation.md) | 실제 실행 환경·시간·품질 점검 범위 |

### 개발자용 검사 명령과 모델 저장 위치 변경
...
TTS 모델을 다른 폴더에 저장하려면 **동일한 `--model-dir`을 다운로드·점검·변환에 모두 지정**합니다. 아래 경로는 원하는 실제 경로로 바꾸세요. 이 옵션은 지정된 Qwen 모델 한 개의 저장 위치이며, 기존 모델을 자동으로 옮기지는 않습니다. 임의의 다른 모델을 지원한다는 뜻도 아닙니다. CLI와 로컬 웹 서버에서 같은 모델 폴더를 관리하려면 [로컬 사용 안내의 공통 저장 폴더 설정](docs/local-usage.md#cli와-로컬-웹-서버가-같은-폴더를-사용하기)을 참고하세요.
