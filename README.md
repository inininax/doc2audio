...
**사이트에 접속하고 문서를 선택하면, 모델 다운로드부터 음성 생성·MP3 저장까지 사용자 PC의 브라우저에서 처리합니다.** 공개 서버에는 웹 정적 파일만 올리면 됩니다. 기본 웹 모드에는 Python 서버, ffmpeg, 별도 실행 프로그램이 필요하지 않습니다. 기존 Mac용 CLI와 로컬 API 서버도 유지합니다.

개발·빌드에는 Node.js 22.20 이상(22.x), 24.12 이상(24.x), 또는 26 이상이 필요합니다. 처음 의존성을 설치할 때는 인터넷이 필요합니다.
...
브라우저에서 **http://127.0.0.1:4173**을 엽니다. `preview`는 로컬 빌드 확인용 서버이며, 배포할 파일은 **`apps/web/dist/` 전체**입니다. HTTPS를 제공하는 정적 호스팅에 올릴 수 있습니다. 서버 설정과 하위 경로 배포는 [브라우저 실행·배포 안내](docs/browser-runtime.md)를 참고하세요.

1. **모델 보관함**에서 Supertonic 3의 정보·라이선스를 확인하고 다운로드합니다. 약 401 MB의 모델이 해당 브라우저에 저장됩니다.
2. **새 음성 만들기**에서 PDF·DOCX·DOC·UTF-8 TXT 파일을 선택하거나 텍스트를 입력합니다.
3. 목소리·언어·발화 속도·출력 배속 등을 고르고 변환을 시작합니다.
4. **작업 기록**에서 진행률을 확인하고, 완료된 MP3를 듣거나 일반 파일로 저장합니다.

**브라우저를 닫으면 계산은 멈추고, 같은 사이트를 같은 브라우저에서 다시 열면 저장된 구간부터 자동으로 이어갑니다.** 닫는 순간 생성하던 미완료 구간은 다시 만들 수 있습니다. 직접 일시정지하거나 실패한 작업은 기록에서 이어하기·재시도를 선택합니다. 원문과 완료 구간을 브라우저 저장소에 보관하므로 파일을 다시 선택할 필요가 없습니다.

화면·문서 처리·OCR 실행 파일의 오프라인 준비와 모델 다운로드를 마치면 인터넷 없이 사용할 수 있습니다. 첫 방문, 누락된 실행 파일 준비, 새 모델 다운로드에는 인터넷이 필요합니다. 브라우저 데이터 삭제·다른 도메인·다른 브라우저 프로필에서는 기존 보관함이 이어지지 않습니다. 완성된 MP3는 별도로 저장하세요.

| 실행 방식 | 지원 모델 | 저장 위치 |
| --- | --- | --- |
| 기본 브라우저 웹 | Supertonic 3 · 10개 목소리 · 한국어 포함 31개 언어 | 해당 사이트의 브라우저 저장소 |
| 선택적 로컬 서버·CLI | Qwen3-TTS 1.7B·0.6B, Supertonic 3 | PC의 `.models/`, `.doc2audio/` 등 |

Supertonic 모델은 사용 조건이 포함된 OpenRAIL-M, Qwen 모델은 Apache-2.0입니다. 목록은 검토한 고정 카탈로그이며 새 모델을 자동 수집하지 않습니다. Qwen의 Python·MLX 구현은 기본 브라우저 모드에서 실행하지 않습니다.

기존 로컬 서버 모드를 쓰려면 Python·모델 등 아래 CLI 준비 사항을 갖춘 뒤 `uv run doc2audio-server`를 실행하고 **http://127.0.0.1:8010/?runtime=local**로 접속합니다. 이 모드에서는 브라우저를 닫아도 Python 서버가 켜져 있으면 작업이 계속됩니다.

[웹 사용 안내](docs/web-usage.md) · [브라우저 실행·배포 안내](docs/browser-runtime.md) · [모노레포 설계](docs/monorepo.md)
