...
아래 예제는 같은 샘플을 사용하므로 별도 문서를 준비할 필요가 없습니다. 원하는 예제만 실행하고 들어 보세요. 화자·언어·말투 등 생성 설정을 바꾸면 음성을 다시 생성합니다. 출력 배속·구간 쉼만 바꾸면 같은 작업 폴더의 유효한 구간을 재사용합니다.
...
속도는 모델 지시가 아니라 ffmpeg의 후처리입니다. 출력 속도·구간 쉼은 음성 생성 캐시에서 제외하므로 같은 작업 폴더에서는 이 값만 바꿀 때 기존 음성 구간을 재사용합니다.
...
옵션의 최종 기준은 아래 도움말과 [CLI 소스](../apps/cli/src/doc2audio_cli/cli.py)입니다.
