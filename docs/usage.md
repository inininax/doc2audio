...
| 언어              | 본문에 맞춰 선택합니다. 기본값은 한국어입니다.                                                       |
| 출력 배속         | 0.5~2배, 기본 1배. 음높이를 유지하면서 완성 파일의 속도를 바꿉니다.                                  |
| 모델 발화 속도    | Supertonic만 지원. 0.7~2배, 기본 1.05배. 음성을 생성할 때 읽는 속도이며 출력 배속과 함께 적용됩니다. |
...
| `--model-options` | 모델별 추가 설정 JSON. 예: Supertonic의 `'{"total_steps": 8, "speech_speed": 1.05}'` |
